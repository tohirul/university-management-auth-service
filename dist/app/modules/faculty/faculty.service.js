...
const faculty_update_embed_1 = __importDefault(require("./faculty.update.embed"));
const getFacultyFromDb = async (id) => {
    return await faculty_model_1.default.findOne({ id })
...
const getAllFacultyFromDb = async (filters, options) => {
    ...
    const result = await faculty_model_1.default.find(searchConditions)
        .populate('academicDepartment')
        .populate('academicFaculty')
        .sort(sortConditions)
        .skip(skip)
...
const updateFacultyInDb = async (id, payload) => {
    ...
    if (name && Object.keys(name).length) {
        updatedFacultyData = (0, faculty_update_embed_1.default)(name, updatedFacultyData, 'name');
    }
    return await faculty_model_1.default.findOneAndUpdate({ id }, updatedFacultyData, {
        new: true,
    })
        .populate('academicDepartment')
        .populate('academicFaculty');
...
const deleteFacultyFromDb = async (id) => {
    ...
    try {
        const deletedFaculty = await faculty_model_1.default.findOneAndDelete({ id }, { session });
        if (!deletedFaculty) {
