...
const student_update_embed_1 = __importDefault(
  require('./student.update.embed')
);
const getStudentFromDb = async id => {
  return await student_model_1.default
    .findById(id)
...
const getAllStudentFromDb = async (filters, options) => {
  ...
  const result = await student_model_1.default
...
const updateStudentInDb = async (id, payload) => {
  ...
  if (name && Object.keys(name).length)
    updatedStudentData = (0, student_update_embed_1.default)(
      name,
      updatedStudentData,
      'name'
    );
  ...
  return await student_model_1.default
    .findOneAndUpdate({ id }, updatedStudentData, {
      new: true,
    })
    .populate('academicSemester')
    .populate('academicDepartment')
