...
const createAcademicDepartmentInDb = async (payload) => {
    return (await academicDepartment_model_1.default.create(payload)).populate('academicFaculty');
};
const getAllAcademicDepartmentsFromDb = async (filters, options) => {
    ...
    const searchConditions = conditions.length > 0
        ? {
            $and: conditions,
        }
        : {};
    const total = await academicDepartment_model_1.default.countDocuments(searchConditions);
    const result = await academicDepartment_model_1.default.find(searchConditions)
...
const updateAcademicDepartmentInDb = async (id, payload) => {
    return await academicDepartment_model_1.default.findOneAndUpdate({ _id: id }, payload, {
        new: true,
    }).populate('academicFaculty');
};
const deleteAcademicDepartmentFromDb = async (id) => {
    return await academicDepartment_model_1.default.findByIdAndDelete(id).populate('academicFaculty');
};
