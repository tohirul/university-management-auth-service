...
const getAllSemestersFromDb = async (filters, options) => {
  ...
  const result = await academicSemester_model_1.default
    .find(searchConditions)
    .sort(sortConditions)
    .skip(skip)
    .limit(limit);
  const total = await academicSemester_model_1.default.countDocuments(
    searchConditions
  );
  return {
    meta: {
...
const updateSemesterFromDb = async (id, payload) => {
  ...
  return await academicSemester_model_1.default.findOneAndUpdate(
    { _id: id },
    payload,
    {
      new: true,
