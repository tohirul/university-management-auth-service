...
const getAllAdmin = async (filters, options) => {
    ...
    const result = await admin_model_1.default.find(searchConditions)
...
const getAdminById = async (id) => {
    return await admin_model_1.default.findOne({ id }).populate('managementDepartment');
};
const updateAdminById = async (id, payload) => {
    ...
    if (name && Object.keys(name).length)
        updatedAdminData = (0, admin_update_embed_1.default)(name, updatedAdminData, 'name');
    return await admin_model_1.default.findOneAndUpdate({ id }, updatedAdminData, {
        new: true,
    }).populate('managementDepartment');
};
const deleteAdmin = async (id) => {
    ...
    try {
        const deletedAdmin = await admin_model_1.default.findOneAndDelete({ id }, { session });
        if (!deletedAdmin)
